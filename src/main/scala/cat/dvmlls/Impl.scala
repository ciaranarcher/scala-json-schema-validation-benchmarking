...
trait JU_HM extends JU [util.HashMap[Int,Double]] { override def create(capacity:Int) = new util.HashMap[Int,Double](capacity) }
trait JU_TM extends JU [util.TreeMap[Int,Double]] { override def create(capacity:Int) = new util.TreeMap[Int,Double]() }
trait JU_C_HM extends JU [util.concurrent.ConcurrentHashMap[Int,Double]] { override def create(capacity:Int) = new util.concurrent.ConcurrentHashMap[Int,Double](capacity) }
...
trait SC_M_HM extends SC_M [mutable.HashMap[Int,Double], Int] { override def create(capacity:Int) = new mutable.HashMap[Int,Double]() }
trait SC_M_OHM extends SC_M [mutable.OpenHashMap[Int,Double], Int] { override def create(capacity:Int) = new mutable.OpenHashMap[Int,Double](capacity) }
trait SC_M_LHM extends SC_M [mutable.LinkedHashMap[Int,Double], Int] { override def create(capacity:Int) = new mutable.LinkedHashMap[Int,Double]() }
trait SC_M_LsM extends SC_M [mutable.ListMap[Int,Double], Int] { override def create(capacity:Int) = new mutable.ListMap[Int,Double]() }
trait SC_M_LoM extends SC_M [mutable.LongMap[Double], Long] { override def create(capacity:Int) = new mutable.LongMap[Double](capacity) }

trait SC_I[T <: immutable.Map[K,Double], K] extends Impl[T, K] {
  var map = create(capacity:Int)
  override def get(n:K):Double = map.get(n).get
}
